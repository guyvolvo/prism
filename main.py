...
def resolve_api_key(args_api):
    ...
    if isinstance(args_api, str):
        ...
        with print_lock:
            ...
            print(f"[*] Saving API Key in {env_path} ...{PC.RESET}")
        try:
            ...
            with print_lock:
                print(f"{PC.WARNING}[!] Error saving key: {e}{PC.RESET}")
            return clean_key
...
def process_file_worker(file_path, args, api_key, stats, results_log):
    try:
        ...
        if file_size > MAX_FILE_SIZE and not args.large:
            with print_lock:
                print(f"{PC.WARNING}[!] Skipping {os.path.basename(file_path)}: > 100MB{PC.RESET}")
        ...
        if args.metadata:
            print_metadata_only(file_path, sha256, md5)
            if not args.scan: return

        scanner = get_scanner()
        parser_data = triage_router(file_path) or {"Triggers": [], "Status": "Error"}
        triage_data = triage(file_path=file_path, data=raw_bytes, scanner=scanner, api_key=api_key)

        if not triage_data:
            triage_data = {"Status": "CLEAN", "YARA_Matches": [], "Heuristics": [], "Score": 0}

        triage_data["Triggers"] = (triage_data.get("YARA_Matches") or []) + (triage_data.get("Heuristics") or [])

        report = {
            "file_info": {"name": os.path.basename(file_path), "path": file_path, "sha256": sha256, "size": file_size},
...
def main():
    ...
    if args.api and not args.target: return
    ...
    print(f"{PC.INFO}[*] Prism Threaded Engine Ready. Workers: {args.threads} | Targets: {len(files_to_process)}\n")
    ...
    try:
        ...
        print(f"\n{PC.WARNING}[!] Interrupt received. Cleaning up...{PC.RESET}")
    ...
    if args.log:
        ...
        print(f"\n{PC.SUCCESS}[+] Log saved: {args.log}{PC.RESET}")
